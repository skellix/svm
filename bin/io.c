...
StackItem* openStream(char* address, char* mode) {
	...
	if (colon) {
		...
		if (strcmp(address, "http") == 0) {
			...
			free(message);
			int* desc = calloc(1, sizeof(int) + sizeof(void*) + sizeof(void*) + sizeof(void*));
			*desc = socketDisc;
			void* lib = dlopen("libsvm.io.so", RTLD_LAZY);
			((void**) (desc + 1))[0] = dlsym(lib, "readStream");
			((void**) (desc + 1))[1] = dlsym(lib, "closeStream");
			((void**) (desc + 1))[2] = lib;
			if (dlerror()) {
				errorMessage = "failed to load libsvm.io.so";
				longjmp(errBuf, 1);
			}
		...
		FILE* file;
		if (access(address, F_OK) == -1 && write == 0) {
			errorMessage = realloc(errorMessage, (strlen(address) + 24) * sizeof(char));
			sprintf(errorMessage, "file '%s' does not exist", address);
			longjmp(errBuf, 1);
		}
